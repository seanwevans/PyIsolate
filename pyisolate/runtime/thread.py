"""Sandbox thread implementation -- the ``backend="thread"`` runtime.

This executes guest code in a dedicated :class:`threading.Thread` of the
supervisor's own process, using the standard interpreter: guest source is
``exec``'d against a restricted ``__builtins__`` mapping with thread-local
policy state. It is not a security boundary against hostile Python --
adversarial code can walk ``object.__subclasses__()`` to recover the real
``__import__`` and reach ``os``/``open``. See ``docs/threat-model.md`` for the
normative boundary statement.

The backend this module implements used to be spelled ``"subinterpreter"``,
which described an implementation it did not have. That spelling still works
and warns; see ``DEPRECATED_BACKEND_ALIASES`` in :mod:`pyisolate.supervisor`.
